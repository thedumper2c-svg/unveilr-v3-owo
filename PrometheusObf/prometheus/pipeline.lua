...
function Pipeline:fromConfig(config)
	...
	for i, step in ipairs(steps) do
		...
		-- if antitamper is on, both work
		-- if antitamper is off, only non antitamper works

		local alias = aliases[step.Name]
		local allow = config.Settings[alias] -- {anti-tamper = false | true}

		if allow ~= false then -- if step.Name == "AntiTamper" and allowAntiTamper or step.Name ~= "AntiTamper"
